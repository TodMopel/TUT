...
namespace TodMopel
{
	[CreateAssetMenu(fileName = "FixWrapFromHangAction FixHangPointAction", menuName = "Tod Unity ToolBox/Movements/FixMovement/FixHangPoint Actions/FixWrapFromHangAction", order = 30)]
	public class FixWrapFromHangAction : AbstractHangPointAction
	{
		...
		{
			Vector2 ControlPositionOffset = OffsetFromSegmentPositionToConnectedBody();
			RaycastHit2D WrapControlRay = Physics2D.Raycast(ControlPositionOffset, fixHangPointComponent.CollectInBetweenRopeDirection(), fixHangPointComponent.CollectInBetweenRopeDistance(), fixHangPointComponent.PlateformLayer);
		...
		private Vector2 OffsetFromSegmentPositionToConnectedBody() => fixHangPointComponent.GetCurrentFixSegmentPointPosition() + (fixHangPointComponent.GetHoldConnectedBodyPosition() - fixHangPointComponent.GetCurrentFixSegmentPointPosition()).normalized * fixWrapOffsetDistance;
